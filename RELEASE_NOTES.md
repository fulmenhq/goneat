...
- `.goneat/tools.yaml` replaces every hidden default—generate it with `goneat doctor tools init` (or `--minimal` for CI) and commit the result (no manual editing required).
- Package manager workflows now prefer sudo-free installers, support user-local Homebrew bootstraps, and automatically extend PATH on GitHub Actions runners.
- Cooling-policy metadata and repo mappings live in configuration files with schema validation, and Go tool version checks use lexical parsing for accuracy.
- `scripts/verify-public-key.sh` blocks accidental publishing of private signing keys and is wired into the release checklist.

## Highlights

### Foundation tools become explicit configuration
- `goneat doctor tools init` (single command, no manual editing) seeds `.goneat/tools.yaml` using repository language detection with `--scope`, `--language`, `--minimal`, and `--force` controls.
- New SSOT files (`config/tools/foundation-tools-defaults.yaml`, `config/tools/foundation-package-managers.yaml`) plus schemas keep defaults versioned and reviewable.
- Hardcoded helpers (`KnownInfrastructureTools`, `ensureCoreScopes`, etc.) were removed; doctor/assess now read only the committed config.
- `cmd/doctor_tools_init_test.go` and related package tests cover success, minimal, overwrite, and error flows.

### CI-friendly package manager & PATH workflows
- Package manager detection surfaces eight managers, reports sudo requirements, and prefers user-local Homebrew installs (`~/homebrew`) when brew must be bootstrapped.
- Automatic PATH management extends the CLI and `$GITHUB_PATH` so mise, bun, scoop, and go-install shims work immediately after `goneat doctor tools --install`.
- Go tool detection now uses lexical version parsing, preventing false negatives when policy-checking `go version go1.x.y` outputs.

### Tool metadata & release safeguards
- Cooling policy features read GitHub repo mappings from `config/tools/common-tools-repos.yaml` (embedded + schema validated) instead of hardcoded maps.
- `scripts/verify-public-key.sh` adds layered negative/positive grep checks plus `gpg --show-keys` parsing, and `RELEASE_CHECKLIST.md` now requires running it before publishing public keys.

## Breaking Changes

- `.goneat/tools.yaml` is required—commands error if it is missing or uninitialized.
- Foundation tools are no longer injected automatically; every repository (local or CI) must run `goneat doctor tools init` and commit the file.
- PATH activation now happens through goneat’s shim management; custom scripts should drop manual shim exports.

## Upgrade Guide

1. Install goneat v0.3.7: `go install github.com/fulmenhq/goneat@v0.3.7`.
2. In each repository run `goneat doctor tools init` (local) or `goneat doctor tools init --minimal` (CI) and commit `.goneat/tools.yaml`.
3. Update CI to call `goneat doctor tools --install --yes`; PATH updates happen automatically on runners.
4. Release managers must run `scripts/verify-public-key.sh <public-key.asc>` before distributing signing keys.

## Documentation

- `docs/guides/goneat-tools-cicd-runner-support.md` – zero-touch PATH activation on runners.
- `docs/reference/tools-packaging-ci-runner-reference.md` – package manager research and sudo expectations.
- `docs/appnotes/tools-runner-usage.md` – migration steps for the new init workflow.
