...
case "$OS" in
  linux) EXT=tar.gz;;
  darwin) EXT=tar.gz;;
  msys*|mingw*|cygwin*) echo "Use install.ps1 on Windows"; exit 1;;
...
( cd "$TMPDIR" && shasum -a 256 -c SHA256SUMS 2>/dev/null | grep " $ASSET: OK" >/dev/null ) || {
  ...
  [ "$SUM_EXPECTED" = "$SUM_ACTUAL" ] || { echo "Checksum mismatch" >&2; exit 1; }
}
...
BIN_NAME="$PROJECT"
[ -f "$TMPDIR/$BIN_NAME" ] || { echo "Binary $BIN_NAME not found in archive" >&2; exit 1; }
