...
var docsListCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        err := fs.WalkDir(dfs, "docs", func(p string, d fs.DirEntry, err error) error {
            ...
            if !strings.HasSuffix(p, ".md") { return nil }
        ...
        fmt.Fprintln(cmd.OutOrStdout(), "Hint: use 'goneat docs show <slug>', e.g., 'user-guide/commands/format'.")
...
var docsShowCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        f, err := dfs.Open(p)
        if err != nil { return fmt.Errorf("doc not found: %s", slug) }
        defer f.Close()
        if docsOutputFormat == "json" {
            ...
            b := new(strings.Builder)
            sc := bufio.NewScanner(f)
            for sc.Scan() { b.WriteString(htmlEscape(sc.Text())); b.WriteByte('\n') }
            ...
            if docsOpen {
                ...
                if err != nil { return err }
                defer tmp.Close()
                if _, err := tmp.WriteString(html); err != nil { return err }
        ...
        sc := bufio.NewScanner(f)
        for sc.Scan() { fmt.Fprintln(cmd.OutOrStdout(), sc.Text()) }
        return sc.Err()
    },
}
...
func sizeOfFS(fi fs.FileInfo) int64 { if fi == nil { return 0 }; return fi.Size() }
...
func printDocsTree(w io.Writer, items []docListItem) {
    ...
    for _, it := range items {
        ...
        cur := root
        for i, p := range parts {
            if cur.kids == nil { cur.kids = map[string]*node{} }
            if _, ok := cur.kids[p]; !ok {
                cur.kids[p] = &node{name: p, kids: map[string]*node{}}
            }
            cur = cur.kids[p]
            if i == len(parts)-1 { cur.file = true; cur.size = it.Size }
        }
    }
    ...
    walk = func(n *node, prefix string) {
        ...
        for k := range n.kids { names = append(names, k) }
        sort.Strings(names)
        for i, name := range names {
            ...
            connector := "├── "
            nextPrefix := prefix + "│   "
            if last { connector = "└── "; nextPrefix = prefix + "    " }
            if child.file {
